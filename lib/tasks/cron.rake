...
task :cron => :environment do
    ...
    Guild.find(:all, :conditions => [ "fetched_at < ?", Time.now - 30.minutes ], :order => "fetched_at" ).each{|g|
    ...
    Character.find(:all, :conditions => [ "fetched_at < ?", Time.now - 2.hours ], :order => "fetched_at" ).each{|c|
